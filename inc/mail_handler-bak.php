<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        if (empty($_POST["terms"]) || empty($_POST["privacyterms"]) || empty($_POST["consignterms"]) || empty($_POST["paylaterterms"])) {
            $_SESSION['termsreqErr'] = "필수 약관에 동의해 주세요.";
        }

        // validate phone identification
        $impField = $_POST['impField'];
        if (empty($impField)) {
            $_SESSION['phoneValErr'] = "휴대폰 본인인증을 진행해 주세요.";
        } else {
            $_SESSION['phoneValSuccess'] = "휴대폰 본인인증이 성공적으로 완료되었습니다.";
        }

        // validate email
        if (empty($_POST["email"])) {
            $_SESSION['emailErr'] = "재학중인 대학교 이메일 주소를 입력해 주세요.";
           } else {
            $email = form_input($_POST["email"]);
            // check if e-mail address is well-formed
            if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            $_SESSION['emailErr'] = "올바르지 않은 이메일 형식입니다. 다시 입력해 주세요.";
            }
        }
        ...
        if (empty($_POST["website"])) {
            $_SESSION['websiteErr'] = "구매 상품의 URL 주소를 입력해 주세요.";
        } else {
            $website = form_input($_POST["website"]);
            // check if URL address syntax is valid (this regular expression also allows dashes in the URL)
            if (!preg_match("/\b(?:(?:https?|ftp):\/\/|www\.)[-a-z0-9+&@#\/%?=~_|!:,.;]*[-a-z0-9+&@#\/%=~_|]/i",$website)) {
            $_SESSION['websiteErr'] = "올바르지 않은 URL 형식입니다. 다시 입력해 주세요.";
            }
        }

        // validate product name
        if (empty($_POST["prdName"])) {
            $_SESSION['prdNameErr'] = "제품이름을 입력해 주세요.";
        } else {
        ...
        if (empty($_POST["prdPrice"])) {
            $_SESSION['prdPriceErr'] = "제품가격을 입력해 주세요.";
        } else {
        ...
        if (empty($_POST["delType"])) {
            $_SESSION['delTypeErr'] = "배송옵션을 선택해 주세요.";
        } else {
        ...
        if (empty($_POST["delCost"])) {
            $_SESSION['delCostErr'] = "배송비를 입력해 주세요.";
        } else {
        ...
        if (empty($_POST["postCode"] || empty($_POST["roadAdd"] || empty($_POST["jibunAdd"])))) {
            $_SESSION['addErr'] = "우편번호와 도로명/지번주소는 필수 입력사항 입니다.";
        } else {
        ...
        if (empty($_POST["payDay"])) {
            $_SESSION['payDayErr'] = "희망 Pay Day를 입력해 주세요.";
        } else {
            ...
            if (!checkdate($payDayArr[0], $payDayArr[1], $payDayArr[2])) {
                $_SESSION['payDayErr'] = "올바르지 않은 날짜 형식입니다. 다시 입력해 주세요.";
            }
        }

        // validate error before proceed
        if ($_SESSION['termsErr'] !='' || $_SESSION['phoneValErr'] !='' || $_SESSION['emailErr'] !='' || $_SESSION['attachmentErr'] !='' || $_SESSION['websiteErr'] !='' || $_SESSION['prdNameErr'] !='' || $_SESSION['prdPriceErr'] !='' || $_SESSION['delTypeErr'] !='' || $_SESSION['delCostErr'] !='' || $_SESSION['payDayErr'] !='') {
        } else {
